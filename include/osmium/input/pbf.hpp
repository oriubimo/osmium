                    ...
                    while (read_blob_header()) {
                        ...
                        if (m_pbf_blob_header.type() == "OSMData") {
                            ...
                            if (pbf_header_block.has_bbox()) {
                                ...
                                this->meta().bounds().extend(Osmium::OSM::Position((int64_t)bbox.left()  / resolution_convert, (int64_t)bbox.bottom() / resolution_convert));
                                this->meta().bounds().extend(Osmium::OSM::Position((int64_t)bbox.right() / resolution_convert, (int64_t)bbox.top()    / resolution_convert));
                            }
                        } else {
            ...
            */
            ...
            void parse_node_group(const OSMPBF::PrimitiveGroup& group, const OSMPBF::StringTable& stringtable, T) {
                ...
                for (int entity=0; entity < max_entity; ++entity) {
                    ...
                    node.position(Osmium::OSM::Position(
                                      (int64_t)(pbf_node.lon() * m_pbf_primitive_block.granularity() + m_pbf_primitive_block.lon_offset()) / (OSMPBF::lonlat_resolution / Osmium::OSM::coordinate_precision),
                                      (int64_t)(pbf_node.lat() * m_pbf_primitive_block.granularity() + m_pbf_primitive_block.lat_offset()) / (OSMPBF::lonlat_resolution / Osmium::OSM::coordinate_precision)));
            ...
            void parse_dense_node_group(const OSMPBF::PrimitiveGroup& group, const OSMPBF::StringTable& stringtable, T) {
                ...
                for (int entity=0; entity < max_entity; ++entity) {
                    ...
                    node.position(Osmium::OSM::Position(
                                      (int64_t)(last_dense_longitude * m_pbf_primitive_block.granularity() + m_pbf_primitive_block.lon_offset()) / (OSMPBF::lonlat_resolution / Osmium::OSM::coordinate_precision),
                                      (int64_t)(last_dense_latitude  * m_pbf_primitive_block.granularity() + m_pbf_primitive_block.lat_offset()) / (OSMPBF::lonlat_resolution / Osmium::OSM::coordinate_precision)));
